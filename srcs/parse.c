...
/*   Updated: 2025/01/30 22:29:44 by dfeve            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_pile	*parse_str(char *str)
{
	...
	while (split[i])
	{
		if (((split[i][0] == '-' || split[i][0] == '+') && !split[i][1]) || check_num(split[i]) == -1)
			error("Need a number as argument", result);
		if (ft_atoi(split[i]) > INT_MAX || ft_atoi(split[i]) < INT_MIN || ft_strlen((const char *)split[i]) >= 20)
...
t_pile	*parse_opt(char **opt)
{
	...
	while (opt[i])
	{
		if (((opt[i][0] == '-' || opt[i][0] == '+')
			...
			error("Need a number as argument", result);
		if (ft_atoi(opt[i]) > INT_MAX || ft_atoi(opt[i]) < INT_MIN || ft_strlen((const char *)opt[i]) >= 20)
			error("Number not in range", result);
